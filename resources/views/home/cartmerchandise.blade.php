...
<body>
    ...
    <div class="quantity-section">
        <div class="player_deg">
            <img class="img_deg" src="/merchandiseimage/{{ $merchandise->image }}" alt="Merchandise Image">
    ...
    <div class="total-section">
        <h2>Total</h2>
        <p id="totalAmount">Total: RM {{ $merchandise->ringgit }} </p>
        <button class="btn btn-primary" onclick="confirmOrder()">Confirm Order</button>
    </div>

    <!-- Pass merchandise price and title from PHP to JavaScript -->
    <script>
        var merchandisePrice = {{ $merchandise->ringgit }};
        var merchandiseTitle = "{{ $merchandise->title }}";

        function calculateTotal() {
            // Get the quantity from the input field
            var quantity = document.getElementById('quantity').value;

            // Calculate the total based on the quantity and merchandise price
            var total = quantity * merchandisePrice;
        ...
        function confirmOrder() {
            ...
            Swal.fire({
                ...
                text: `You have successfully ordered ${quantity} shirt(s) of "${merchandiseTitle}" for a total of RM ${quantity * merchandisePrice}.`,
